...
public class OpenHashing <AnyType extends Comparable<? super AnyType>> {
	...
	public void delete(String deleteString) throws IllegalArgumentException{
	 ...
	 */
		...
		for (int i = 0; i < deleteString.length(); i++) {
		    hash = hash*31 + deleteString.charAt(i);
		    hash = hash%4007;
		}
		
		
		Node myNode = new Node(deleteString);
		SinglyLinkedList hashedSLL = hashList.get(hash);
